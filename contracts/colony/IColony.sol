...
abstract contract IColony is ColonyDataTypes, IRecovery {
  ...
  /// @return colonyAuthority The `ColonyAuthority` abstract contract address
  function authority() public view virtual returns (address colonyAuthority);
  ...
  function owner() public view virtual returns (address colonyOwner);

  // Implemented in Colony.sol
  /// @notice Get the Colony abstract contract version.
  /// Starts from 1 and is incremented with every deployed abstract contract change.
  /// @return colonyVersion Version number
  function version() public pure virtual returns (uint256 colonyVersion);

  /// @notice Upgrades a colony to a new Colony abstract contract version `_newVersion`.
  ...
  function upgrade(uint _newVersion) public virtual;
  ...
  function getColonyNetwork() public view virtual returns (address colonyNetwork);

  /// @notice Get the colony token.
  /// @return tokenAddress Address of the token abstract contract
  function getToken() public view virtual returns (address tokenAddress);
  ...
  function setRootRole(address _user, bool _setTo) public virtual;
  ...
  /// @param _setTo The state of the role permission (true assign the permission, false revokes it)
  function setArbitrationRole(uint256 _permissionDomainId, uint256 _childSkillIndex, address _user, uint256 _domainId, bool _setTo) public virtual;
  ...
  /// @param _domainId Domain in which we are giving user the role
  /// @param _setTo The state of the role permission (true assign the permission, false revokes it)
  function setArchitectureRole(uint256 _permissionDomainId, uint256 _childSkillIndex, address _user, uint256 _domainId, bool _setTo) public virtual;
  ...
  /// @param _domainId Domain in which we are giving user the role
  /// @param _setTo The state of the role permission (true assign the permission, false revokes it)
  function setFundingRole(uint256 _permissionDomainId, uint256 _childSkillIndex, address _user, uint256 _domainId, bool _setTo) public virtual;
  ...
  /// @param _domainId Domain in which we are giving user the role
  /// @param _setTo The state of the role permission (true assign the permission, false revokes it)
  function setAdministrationRole(uint256 _permissionDomainId, uint256 _childSkillIndex, address _user, uint256 _domainId, bool _setTo) public virtual;

  /// @notice Check whether a given user has a given role for the colony.
  /// Calls the function of the same name on the colony's authority abstract contract.
  ...
  function hasUserRole(address _user, uint256 _domainId, ColonyRole _role) public view virtual returns (bool hasRole);

  /// @notice Check whether a given user has a given role for the colony, in a child domain.
  /// Calls the function of the same name on the colony's authority abstract contract and an internal inheritence validator function
  /// @param _user The user whose role we want to check
  ...
  function hasInheritedUserRole(address _user, uint256 _domainId, ColonyRole _role, uint256 _childSkillIndex, uint256 _childDomainId)
    public view virtual returns (bool hasRole);
  ...
  function getUserRoles(address who, uint256 where) public view virtual returns (bytes32 roles);
  ...
  function initialiseColony(address _colonyNetworkAddress, address _token) public virtual;
  ...
  function bootstrapColony(address[] memory _users, int[] memory _amount) public virtual;

  /// @notice Mint `_wad` amount of colony tokens. Secured function to authorised members.
  /// @param _wad Amount to mint
  function mintTokens(uint256 _wad) public virtual;
  ...
  function registerColonyLabel(string memory colonyName, string memory orbitdb) public virtual;
  ...
  function updateColonyOrbitDB(string memory orbitdb) public virtual;
  ...
  function getDomain(uint256 _id) public view virtual returns (Domain memory domain);
  ...
  function getDomainCount() public view virtual returns (uint256 count);
  ...
  function makeExpenditure(uint256 _permissionDomainId, uint256 _childSkillIndex, uint256 _domainId) public virtual returns (uint256 expenditureId);
  ...
  function transferExpenditure(uint256 _id, address _newOwner) public virtual;
  ...
  /// @param _id Expenditure identifier
  /// @param _newOwner New owner of expenditure
  function transferExpenditureViaArbitration(uint256 _permissionDomainId, uint256 _childSkillIndex, uint256 _id, address _newOwner) public virtual;

  /// @notice Cancels the expenditure and prevents further editing. Can only be called by expenditure owner.
  /// @param _id Expenditure identifier
  function cancelExpenditure(uint256 _id) public virtual;

  /// @notice Finalizes the expenditure and prevents further editing. Can only be called by expenditure owner.
  /// @param _id Expenditure identifier
  function finalizeExpenditure(uint256 _id) public virtual;
  ...
  function setExpenditureRecipient(uint256 _id, uint256 _slot, address payable _recipient) public virtual;
  ...
  function setExpenditurePayout(uint256 _id, uint256 _slot, address _token, uint256 _amount) public virtual;
  ...
  function setExpenditureSkill(uint256 _id, uint256 _slot, uint256 _skillId) public virtual;
  ...
  /// @param _slot Number of the slot
  /// @param _token Address of the token, `0x0` value indicates Ether
  function claimExpenditurePayout(uint256 _id, uint256 _slot, address _token) public virtual;

  /// @notice Get the number of expenditures in the colony.
  /// @return count The expenditure count
  function getExpenditureCount() public view virtual returns (uint256 count);

  /// @notice Returns an existing expenditure.
  /// @param _id Expenditure identifier
  /// @return expenditure The expenditure
  function getExpenditure(uint256 _id) public view virtual returns (Expenditure memory expenditure);

  /// @notice Returns an existing expenditure slot.
  /// @param _id Expenditure identifier
  /// @param _slot Expenditure slot
  /// @return expenditureSlot The expenditure slot
  function getExpenditureSlot(uint256 _id, uint256 _slot) public view virtual returns (ExpenditureSlot memory expenditureSlot);
  ...
  function getExpenditureSlotPayout(uint256 _id, uint256 _slot, address _token) public view virtual returns (uint256 amount);
  ...
  function addPayment(
    ...
    uint256 _domainId,
    uint256 _skillId)
    public virtual returns (uint256 paymentId);
  ...
  function finalizePayment(uint256 _permissionDomainId, uint256 _childSkillIndex, uint256 _id) public virtual;
  ...
  /// @param _id Payment identifier
  /// @param _recipient Address of the payment recipient
  function setPaymentRecipient(uint256 _permissionDomainId, uint256 _childSkillIndex, uint256 _id, address payable _recipient) public virtual;
  ...
  /// @param _id Payment identifier
  /// @param _skillId Id of the new skill to set
  function setPaymentSkill(uint256 _permissionDomainId, uint256 _childSkillIndex, uint256 _id, uint256 _skillId) public virtual;
  ...
  /// @param _token Address of the token, `0x0` value indicates Ether
  /// @param _amount Payout amount
  function setPaymentPayout(uint256 _permissionDomainId, uint256 _childSkillIndex, uint256 _id, address _token, uint256 _amount) public virtual;

  /// @notice Returns an exiting payment.
  /// @param _id Payment identifier
  /// @return payment The Payment data structure
  function getPayment(uint256 _id) public view virtual returns (Payment memory payment);
  ...
  /// @param _id Payment identifier
  /// @param _token Address of the token, `0x0` value indicates Ether
  function claimPayment(uint256 _id, address _token) public virtual;

  /// @notice Get the number of payments in the colony.
  /// @return count The payment count
  function getPaymentCount() public view virtual returns (uint256 count);
  ...
  function makeTask(
    ...
    uint256 _domainId,
    uint256 _skillId,
    uint256 _dueDate) public virtual;

  /// @notice Get the number of tasks in the colony.
  /// @return count The task count
  function getTaskCount() public view virtual returns (uint256 count);
  ...
  function getTaskChangeNonce(uint256 _id) public view virtual returns (uint256 nonce);
  ...
  function submitTaskWorkRating(uint256 _id, uint8 _role, bytes32 _ratingSecret) public virtual;
  ...
  function revealTaskWorkRating(uint256 _id, uint8 _role, uint8 _rating, bytes32 _salt) public virtual;
  ...
  function generateSecret(bytes32 _salt, uint256 _value) public pure virtual returns (bytes32 secret);
  ...
  function getTaskWorkRatingSecretsInfo(uint256 _id) public view virtual returns (uint256 nSecrets, uint256 lastSubmittedAt);

  /// @notice Get the rating secret submitted for role `_role` in task `_id`
  /// @param _id Id of the task
  /// @param _role Id of the role, as defined in TaskRole enum
  /// @return secret Rating secret `bytes32` value
  function getTaskWorkRatingSecret(uint256 _id, uint8 _role) public view virtual returns (bytes32 secret);
  ...
  /// @param _childSkillIndex The index that the `_domainId` is relative to `_permissionDomainId`
  function setTaskManagerRole(uint256 _id, address payable _user, uint256 _permissionDomainId, uint256 _childSkillIndex) virtual public;
  ...
  function setTaskEvaluatorRole(uint256 _id, address payable _user) public virtual;
  ...
  function setTaskWorkerRole(uint256 _id, address payable _user) public virtual;

  /// @notice Removing evaluator role.
  /// Agreed between manager and currently assigned evaluator.
  /// @param _id Id of the task
  function removeTaskEvaluatorRole(uint256 _id) public virtual;

  /// @notice Removing worker role.
  /// Agreed between manager and currently assigned worker.
  /// @param _id Id of the task
  function removeTaskWorkerRole(uint256 _id) public virtual;
  ...
  function setTaskSkill(uint256 _id, uint256 _skillId) public virtual;
  ...
  function setTaskBrief(uint256 _id, bytes32 _specificationHash) public virtual;
  ...
  function setTaskDueDate(uint256 _id, uint256 _dueDate) public virtual;
  ...
  function submitTaskDeliverable(uint256 _id, bytes32 _deliverableHash) public virtual;
  ...
  function submitTaskDeliverableAndRating(uint256 _id, bytes32 _deliverableHash, bytes32 _ratingSecret) public virtual;
  ...
  /// @param _id Id of the task
  function finalizeTask(uint256 _id) public virtual;
  ...
  /// @param _id Id of the task
  function cancelTask(uint256 _id) public virtual;
  ...
  /// @param _id Id of the task
  function completeTask(uint256 _id) public virtual;
  ...
  function getTask(uint256 _id) public view virtual returns (
  ...
  function getTaskRole(uint256 _id, uint8 _role) public view virtual returns (Role memory role);
  ...
  function setRewardInverse(uint256 _rewardInverse) public virtual;
  ...
  function getRewardInverse() public view virtual returns (uint256 rewardInverse);
  ...
  function getTaskPayout(uint256 _id, uint8 _role, address _token) public view virtual returns (uint256 amount);

  /// @notice Set `_token` payout for manager in task `_id` to `_amount`.
  /// @param _id Id of the task
  /// @param _token Address of the token, `0x0` value indicates Ether
  /// @param _amount Payout amount
  function setTaskManagerPayout(uint256 _id, address _token, uint256 _amount) public virtual;

  /// @notice Set `_token` payout for evaluator in task `_id` to `_amount`.
  /// @param _id Id of the task
  /// @param _token Address of the token, `0x0` value indicates Ether
  /// @param _amount Payout amount
  function setTaskEvaluatorPayout(uint256 _id, address _token, uint256 _amount) public virtual;

  /// @notice Set `_token` payout for worker in task `_id` to `_amount`.
  /// @param _id Id of the task
  /// @param _token Address of the token, `0x0` value indicates Ether
  /// @param _amount Payout amount
  function setTaskWorkerPayout(uint256 _id, address _token, uint256 _amount) public virtual;
  ...
  function setAllTaskPayouts(uint256 _id, address _token, uint256 _managerAmount, uint256 _evaluatorAmount, uint256 _workerAmount) public virtual;
  ...
  /// @param _role Id of the role, as defined in TaskRole enum
  /// @param _token Address of the token, `0x0` value indicates Ether
  function claimTaskPayout(uint256 _id, uint8 _role, address _token) public virtual;
  ...
  /// @param branchMask The branchmask of the proof
  /// @param siblings The siblings of the proof
  function startNextRewardPayout(address _token, bytes memory key, bytes memory value, uint256 branchMask, bytes32[] memory siblings) public virtual;
  ...
  function getRewardPayoutInfo(uint256 _payoutId) public view virtual returns (RewardPayoutCycle memory rewardPayoutCycle);
  ...
  /// @param _payoutId Id of the reward payout
  function finalizeRewardPayout(uint256 _payoutId) public virtual;
  ...
  function getFundingPot(uint256 _id) public view virtual returns (
  ...
  function getFundingPotCount() public view virtual returns (uint256 count);
  ...
  function getFundingPotBalance(uint256 _potId, address _token) public view virtual returns (uint256 balance);

  /// @notice Get the assigned `_token` payouts of pot with id `_potId`.
  /// @param _potId Id of the funding pot
  /// @param _token Address of the token, `0x0` value indicates Ether
  /// @return payout Funding pot payout amount
  function getFundingPotPayout(uint256 _potId, address _token) public view virtual returns (uint256 payout);
  ...
  /// @param _token Address of the token, `0x0` value indicates Ether
  function claimColonyFunds(address _token) public virtual;
  ...
  function getNonRewardPotsTotal(address _token) public view virtual returns (uint256 amount);
}
