...
abstract contract IReputationMiningCycle is ReputationMiningCycleDataTypes {
  ...
  function getDisputeRound(uint256 _round) public view virtual returns (DisputedEntry[] memory submissions);
  ...
  function getReputationHashSubmission(address _user) public view virtual returns (Submission memory submission);
  ...
  function getEntryHash(address submitter, uint256 entryIndex, bytes32 newHash) public pure virtual returns (bytes32 entryHash);
  ...
  function minerSubmittedEntryIndex(address _miner, uint256 _index) public view virtual returns (bool result);
  ...
  function challengeRoundComplete(uint256 round) public view virtual returns (bool complete);
  ...
  /// @notice Get the length of the ReputationUpdateLog stored on this instance of the ReputationMiningCycle abstract contract.
  /// @return nUpdates
  function getReputationUpdateLogLength() public view virtual returns (uint256 nUpdates);
  ...
  function getReputationUpdateLogEntry(uint256 _id) public view virtual returns (ReputationLogEntry memory reputationUpdateLogEntry);
  ...
  function getReputationMiningWindowOpenTimestamp() public view virtual returns (uint256 timestamp);

  /// @notice Initialise this reputation mining cycle.
  /// @dev This will only be called once, by ColonyNetwork, in the same transaction that deploys this abstract contract.
  /// @param tokenLocking Address of the TokenLocking abstract contract
  ...
  function getNUniqueSubmittedHashes() public view virtual returns (uint256 nUniqueSubmittedHashes);
  ...
  function getNInvalidatedHashes() public view virtual returns (uint256 nInvalidatedHashes);

  /// @notice Get the minimum stake of CLNY required to mine.
  /// @return minStake The minimum stake amount
  function getMinStake() public pure virtual returns (uint256 minStake);
  ...
  function getMiningWindowDuration() public pure virtual returns (uint256 miningWindowDuration);
  ...
  function getDecayConstant() public pure virtual returns (uint256 numerator, uint256 denominator);
  ...
  function getSubmissionUser(bytes32 hash, uint256 nNodes, bytes32 jrh, uint256 index) public view virtual returns (address user);
  ...
  function getNSubmissionsForHash(bytes32 hash, uint256 nNodes, bytes32 jrh) public view virtual returns (uint256 count);
}
