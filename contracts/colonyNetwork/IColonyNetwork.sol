...
abstract contract IColonyNetwork is ColonyNetworkDataTypes, IRecovery {
  ...
  /// @notice Query if a abstract contract implements an interface
  /// @param interfaceID The interface identifier, as specified in ERC-165
  /// @dev Interface identification is specified in ERC-165.
  /// @return status `true` if the abstract contract implements `interfaceID`
  function supportsInterface(bytes4 interfaceID) external pure virtual returns (bool status);
  ...
  function getReplacementReputationUpdateLogEntry(address _reputationMiningCycle, uint256 _id) public view virtual returns
  ...
  function getReplacementReputationUpdateLogsExist(address _reputationMiningCycle) public view virtual returns (bool exists);

  /// @notice Get the Meta Colony address.
  /// @return colonyAddress The Meta colony address, if no colony was found, returns 0x0
  function getMetaColony() public view virtual returns (address payable colonyAddress);

  /// @notice Get the number of colonies in the network.
  /// @return count The colony count
  function getColonyCount() public view virtual returns (uint256 count);
  ...
  function isColony(address _colony) public view virtual returns (bool addressIsColony);
  ...
  function addSkill(uint256 _parentSkillId) public virtual returns (uint256 skillId);
  ...
  function getSkill(uint256 _skillId) public view virtual returns (Skill memory skill);

  /// @notice Mark a global skill as deprecated which stops new tasks and payments from using it.
  /// @param _skillId Id of the skill
  function deprecateSkill(uint256 _skillId) public virtual;
  ...
  function appendReputationUpdateLog(address _user, int256 _amount, uint256 _skillId) public virtual;

  /// @notice Get the number of skills in the network including both global and local skills.
  /// @return count The skill count
  function getSkillCount() public view virtual returns (uint256 count);
  ...
  function getReputationMiningSkillId() public view virtual returns (uint256 skillId);

  /// @notice Sets the token locking address.
  /// This is only set once, and can't be changed afterwards.
  /// @param _tokenLockingAddress Address of the locking abstract contract
  function setTokenLocking(address _tokenLockingAddress) public virtual;

  /// @notice Get token locking abstract contract address.
  /// @return lockingAddress Token locking abstract contract address
  function getTokenLocking() public view virtual returns (address lockingAddress);

  /// @notice Create the Meta Colony, same as a normal colony plus the root skill.
  /// @param _tokenAddress Address of the CLNY token
  function createMetaColony(address _tokenAddress) public virtual;
  ...
  function createColony(address _tokenAddress) public virtual returns (address colonyAddress);
  ...
  function createColony(address _tokenAddress, uint256 _version, string memory _colonyName, string memory _orbitdb, bool _useExtensionManager)
    public virtual returns (address colonyAddress);
  ...
  function addColonyVersion(uint256 _version, address _resolver) public virtual;
  ...
  /// @param _resolver Address of the resolver for Colony abstract contract
  /// @param _version Version of the Colony abstract contract the resolver represents
  function initialise(address _resolver, uint256 _version) public virtual;
  ...
  function getColony(uint256 _id) public view virtual returns (address colonyAddress);

  /// @notice Returns the latest Colony abstract contract version. This is the version used to create all new colonies.
  /// @return version The current / latest Colony abstract contract version
  function getCurrentColonyVersion() public view virtual returns (uint256 version);
  ...
  function getParentSkillId(uint256 _skillId, uint256 _parentSkillIndex) public view virtual returns (uint256 skillId);
  ...
  function getChildSkillId(uint256 _skillId, uint256 _childSkillIndex) public view virtual returns (uint256 skillId);
  ...
  function getReputationMiningCycle(bool _active) public view virtual returns (address repMiningCycleAddress);
  ...
  function calculateMinerWeight(uint256 _timeStaked, uint256 _submissonIndex) public pure virtual returns (uint256 minerWeight);

  /// @notice Get the `Resolver` address for Colony abstract contract version `_version`.
  /// @param _version The Colony abstract contract version
  /// @return resolverAddress Address of the `Resolver` abstract contract
  function getColonyVersionResolver(uint256 _version) public view virtual returns (address resolverAddress);

  /// @notice Set a new Reputation root hash and starts a new mining cycle. Can only be called by the ReputationMiningCycle abstract contract.
  ...
  function setReputationRootHash(bytes32 newHash, uint256 newNNodes, address[] memory stakers, uint256 reward) public virtual;
  ...
  function getReputationRootHashNNodes() public view virtual returns (uint256 nNodes);
  ...
  function startTokenAuction(address _token) public virtual;
  ...
  function setupRegistrar(address _ens, bytes32 _rootNode) public virtual;
  ...
  function registerUserLabel(string memory username, string memory orbitdb) public virtual;
  ...
  function updateUserOrbitDB(string memory orbitdb) public virtual;
  ...
  function getProfileDBAddress(bytes32 node) public view virtual returns (string memory orbitDB);
  ...
  function lookupRegisteredENSDomain(address addr) public view virtual returns(string memory domain);
  ...
  function getENSRegistrar() public view virtual returns (address);

  /// @notice Set the resolver to be used by new instances of ReputationMiningCycle.
  /// @param miningResolverAddress The address of the Resolver abstract contract with the functions correctly wired.
  function setMiningResolver(address miningResolverAddress) public virtual;
  ...
  function getMiningResolver() public view virtual returns (address miningResolverAddress);
  ...
  function getFeeInverse() public view virtual returns (uint256 _feeInverse);
  ...
  function setFeeInverse(uint256 _feeInverse) public virtual;
}
