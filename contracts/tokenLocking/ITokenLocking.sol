...
abstract contract ITokenLocking is TokenLockingDataTypes {
  ...
  /// @notice Set the ColonyNetwork abstract contract address.
  ...
  function setColonyNetwork(address _colonyNetwork) public virtual;

  /// @notice Get ColonyNetwork address.
  /// @return networkAddress ColonyNetwork address
  function getColonyNetwork() public view virtual returns (address networkAddress);
  ...
  function lockToken(address _token) public virtual returns (uint256 lockCount);
  ...
  function unlockTokenForUser(address _token, address _user, uint256 _lockId) public virtual;
  ...
  function incrementLockCounterTo(address _token, uint256 _lockId) public virtual;

  /// @notice Deposit `_amount` of colony tokens. Can only be called if user tokens are not locked.
  /// Before calling this function user has to allow that their tokens can be transferred by token locking abstract contract.
  ...
  function withdraw(address _token, uint256 _amount) public virtual;
  ...
  function punishStakers(address[] memory _stakers, address _beneficiary, uint256 _amount) public virtual;

  /// @notice Get global lock count for a specific token.
  /// @param _token Address of the token
  /// @return lockCount Global token lock count
  function getTotalLockCount(address _token) public view virtual returns (uint256 lockCount);
  ...
  function getUserLock(address _token, address _user) public view virtual returns (Lock memory lock);
}
