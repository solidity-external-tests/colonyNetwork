...
abstract contract IPatriciaTreeBase {
  ...
  function getRootHash() public virtual view returns (bytes32 rootHash);

  /// @notice Get the root edge
  /// @return e The root `Data.Edge`
  function getRootEdge() public virtual view returns (Data.Edge memory e);

  /// @notice Get the node with the given key
  /// @param hash The `keccak256` hash of the actual key
  /// @return n `Data.Node` for key `hash`
  function getNode(bytes32 hash) public virtual view returns (Data.Node memory n);
}
