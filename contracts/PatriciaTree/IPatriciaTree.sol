...
abstract contract IPatriciaTree is IPatriciaTreeBase {

  /// @notice Insert the `key`/`value`in the appropriate place in the tree
  function insert(bytes memory key, bytes memory value) public virtual;
  ...
  function getProof(bytes memory key) public virtual view returns (uint branchMask, bytes32[] memory _siblings);
  ...
  function getImpliedRoot(bytes memory key, bytes memory value, uint256 branchMask, bytes32[] memory siblings)
    public virtual pure returns (bytes32 rootHash);
