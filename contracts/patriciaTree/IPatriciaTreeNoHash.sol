...
abstract contract IPatriciaTreeNoHash is IPatriciaTreeBase {
  ...
  function getProof(bytes32 key) public view virtual returns (uint branchMask, bytes32[] memory _siblings);

  /// @notice Calculates and returns a root hash for the `key`, `value`, `branchMask` and `siblings`
  /// @return rootHash The calculated hash
  function getImpliedRoot(bytes32 key, bytes memory value, uint256 branchMask, bytes32[] memory siblings) public pure virtual returns (bytes32 rootHash);
