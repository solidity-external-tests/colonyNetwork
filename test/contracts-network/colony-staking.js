...
contract("Colony Staking", (accounts) => {
  ...
  describe("when managing stakes", () => {
    ...
    it("should not let users obligate more than is approved for obligator", async () => {
      ...
      await checkErrorRevert(colony.obligateStake(USER1, 1, WAD.addn(1), { from: USER0 }), undefined); // FIXME: web3.js does not return the panic code
    });
    ...
    it("should not let users deobligate more than is obligated", async () => {
      ...
      await checkErrorRevert(colony.deobligateStake(USER1, 1, WAD.addn(1), { from: USER0 }), undefined); // FIXME: web3.js does not return the panic code
    });
